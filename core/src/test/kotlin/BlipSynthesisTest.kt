...
class BlipSynthesisTest {

    @Test
    fun addDeltaFastAndReadSamples() {
        val blip = Blip(BLIP_SIZE)

        blip.addDeltaFast(2 * OVERSAMPLE, 16384)
        blip.endFrameAndReadSamples()
    ...
    fun saturation() {
        ...
        blip.readSample(BUF, BLIP_SIZE, false)
        assertEquals(32767, BUF[20].toInt())
        ...
        blip.endFrame(OVERSAMPLE * BLIP_SIZE)
        blip.readSample(BUF, BLIP_SIZE, false)
        assertEquals(-32768, BUF[20].toInt())
    }

    @Test
    fun stereoInterleave() {
        val blip = Blip(BLIP_SIZE)
        ...
        blip.addDelta(0, 16384)
        blip.endFrame(BLIP_SIZE * OVERSAMPLE)
        blip.readSample(STEREO_BUF, BLIP_SIZE, true)

        for (i in 0 until BLIP_SIZE) {
            assertEquals(STEREO_BUF[i * 2], BUF[i])
        }
    }
    ...
    fun clear() {
        ...
        while (n-- > 0) {
            blip.endFrame(BLIP_SIZE * OVERSAMPLE)
            assertEquals(blip.readSample(BUF, BLIP_SIZE, false), BLIP_SIZE)
            BUF.forEach { assertEquals(0, it.toInt()) }
        }
    }

    companion object {
        const val OVERSAMPLE = Blip.BLIP_MAX_RATIO
        const val BLIP_SIZE = 32
        val BUF = ShortArray(BLIP_SIZE)
        val STEREO_BUF = ShortArray(BLIP_SIZE * 2)

        private fun Blip.endFrameAndReadSamples() {
            endFrame(BLIP_SIZE * OVERSAMPLE)
            readSample(BUF, BLIP_SIZE, false)
            clear()
        }
    }
}