...
class Speaker(val console: Console) : AudioDevice {
    ...
    private var emulationSpeed = console.settings.getEmulationSpeed()
    ...
    private fun initializeAudio(sampleRate: Int, emulationSpeed: Int, isStereo: Boolean) {
        stop.set(false)

        if (thread == null) {
            thread = thread(true, name = "Speaker", block = this::renderAudio)
        }
    ...
    override fun play(buffer: ShortArray, length: Int, sampleRate: Int, isStereo: Boolean) {
        updateSoundSettings()

        val bytesPerSample = SoundMixer.BITS_PER_SAMPLE / 8 * if (isStereo) 2 else 1
        writeToBuffer(buffer, length * bytesPerSample)

        pause.set(false)
    }

    private inline val availableBytes: Int
        get() {
            return if (writePosition >= readPosition) {
    ...
    private inline fun updateSoundSettings() {
        ...
        val emulationSpeed = console.settings.getEmulationSpeed()
